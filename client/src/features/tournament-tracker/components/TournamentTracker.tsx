...
import { selectAllMatches } from "../matchesSlice";
...
function TournamentTracker({ liveMatchData }: TournamentTrackerProps) {
  ...
  //   .map((t) => ({
  //     ...t,
  //     matches: t.matches.filter((m) => m.scheduledDate === selectedDate?.iso),
  //   }))
    ...
    >
      ...
      {tournaments.map((t) => {
        ...
        const matches = allMatches.filter(
          (m) =>
            m.tournamentId === t.id && m.scheduledDate === selectedDate?.iso,
        );
        if (matches.length === 0) return null; // hide empty tournaments
        return (
          <TournamentSection
            key={t.id}
