/**
 * Decryption utility for infoSys's live scoring API responses (FOR ROLAND GARROS).
 * Handles AES-CBC decryption.
 */
...
/**
 * Creates a decryption key reverse-engineered from the lastModified field from infoSys's main.js responses
 * @param lastModified 
 * @returns key
 */
function makeKey(lastModified) {
...
/**
/**
 * Recursively replaces obfuscated keys in the given object using the KEY_MAP mapping.
 * Arrays and nested objects are processed to ensure all keys are mapped.
 * 
 * @param {any} obj - The input object or array to transform.
 * @returns {any} A new object or array with keys replaced according to KEY_MAP.
 */
...
/**
 * Decrypts InfoSys Roland Garros API responses for specific fields ('cv', 'cvBelow', 'stroke').
 * Each field is encrypted with AES-CBC using a key derived from its 'lastModified' timestamp.
 * Returns a new object where the fields have been decrypted and parsed to JSON.
 *
 * @param {Object} payload - The response object containing encrypted fields and their 'lastModified' timestamps.
 * @returns {Object} - The response object with decrypted data for the specified fields.
 */
export function decryptResponseRG(payload) {
    // Roland Garros responses have multiple fields with their own unique lastModified
    const fieldsToDecrypt = ['cv', 'cvBelow', 'stroke'];
    const result = { ...payload };

    for (const field of fieldsToDecrypt) {
        const k = makeKey(payload[field].lastModified);
        const dec = CryptoJS.AES.decrypt(payload[field].response, CryptoJS.enc.Utf8.parse(k),
            { 
                iv: CryptoJS.enc.Utf8.parse(k.toUpperCase()),
                mode: CryptoJS.mode.CBC,
                padding: CryptoJS.pad.Pkcs7
            });
        result[field] = remapKeys(JSON.parse(dec.toString(CryptoJS.enc.Utf8)));
...
/**
 * Decrypts a single InfoSys Roland Garros API response.
 * Expects an object with 'lastModified' and 'response' fields.
 * Decrypts the response using a key derived from 'lastModified'.
 *
 * @param {Object} payload - The response object containing 'lastModified' and 'response'.
 * @returns {any} - The decrypted and parsed response object.
 */
export function decryptResponse(payload) {
    const k = makeKey(payload.lastModified);
    const dec = CryptoJS.AES.decrypt(payload.response, CryptoJS.enc.Utf8.parse(k),
        { 
            iv: CryptoJS.enc.Utf8.parse(k.toUpperCase()),
            mode: CryptoJS.mode.CBC,
            padding: CryptoJS.pad.Pkcs7
        });
    const result = remapKeys(JSON.parse(dec.toString(CryptoJS.enc.Utf8)));
